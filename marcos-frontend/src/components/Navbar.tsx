    ...
    return (
        <nav className="sticky top-0 z-50 bg-white border-b-2 border-black">
            <div className="px-6 py-4 flex justify-between items-start md:items-center gap-6 relative z-50 bg-white">
                {/* Heavy stacked logo */}
                <Link href="/" onClick={closeMenu} className="font-poetsen font-normal text-4xl md:text-6xl uppercase tracking-tighter leading-[0.85] flex flex-col">
                ...
                {/* Desktop Navigation */}
                <div className="hidden md:flex flex-wrap gap-6 text-xl font-poetsen font-normal tracking-widest uppercase mt-4 md:mt-0">
            ...
            {isOpen && (
                <div className="md:hidden absolute top-full left-0 w-full bg-white border-b-2 border-black flex flex-col px-6 py-8 gap-6 shadow-xl z-40">
                    <Link href="/#gallery" onClick={closeMenu} className="text-2xl font-poetsen font-normal uppercase tracking-widest hover:text-accent transition-colors">
                        Art Work
                    </Link>
                    <Link href="/about" onClick={closeMenu} className="text-2xl font-poetsen font-normal uppercase tracking-widest hover:text-accent transition-colors">
                        About Me
                    </Link>
                    <Link href="/contact" onClick={closeMenu} className="text-2xl font-poetsen font-normal uppercase tracking-widest hover:text-accent transition-colors">
                        Contact
                    </Link>
                </div>
            )}
